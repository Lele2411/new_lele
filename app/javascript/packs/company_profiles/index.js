$(function() {
    ...
    preventEnter($("#company_profile_phone_number"), lengthOfCompanyPhoneNumber, $("#phone_number_error"), "#phone_number_error_detail", "<div id='phone_number_error_detail'>Phone number must have 10 or 11 characters!</div>");
    ...
    $("#company_profile_phone_number").bind("paste", function(e) {
        if (!e.originalEvent.clipboardData.getData('text').match(/^\d+$/)) {
            e.preventDefault();
        }
    })

    $("#company_profile_phone_number").bind("keypress", function(e) {
        if (!e.key.match(/^\d+$/)) {
            e.preventDefault();
        }
    })
